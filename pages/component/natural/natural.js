...
Page({
     ...
     */
    data: {
        status: 0, //0是少儿区，1是成人区
        list: [],
        hiddenmodalput: true ,//可以通过hidden是否掩藏弹出框的属性，来指定那个弹出框
        imageUrl:'https://herongnizi.cn/study/data/img?path=',
    },
    ...
    onChangeTap: function(event) {
        let that = this;
        let id = event.currentTarget.dataset.status;
        this.setData({
            status: event.currentTarget.dataset.status,
        })
        that.getNaturalData(id);
    },
    // 显示弹框
